...
const ServicesPage = () => {
  ...
  if (showRegistrationForm) {
    return <RegistrationForm serviceType={selectedServiceType} onBack={() => setShowRegistrationForm(false)} />
  }
        ...
        <div className="container">
          ...
          <div className="max-w-6xl mx-auto">
            {activeTab === 'legal' && (
              <div>
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-bold mb-4">Dịch vụ hành chính pháp lý</h2>
            ...
            {activeTab === 'civil' && (
              <div>
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-bold mb-4">Dịch vụ dân sự tự nguyện</h2>
                  <p className="text-lg text-gray-600 max-w-3xl mx-auto">
...
const RegistrationForm = ({ serviceType, onBack }: { serviceType: 'legal' | 'civil', onBack: () => void }) => {
  ...
  const services = serviceType === 'legal' 
    ? ["Xét nghiệm ADN Cha – Con", "Xét nghiệm ADN Mẹ – Con", "Xét nghiệm ADN Họ hàng dòng cha", "Xét nghiệm ADN Họ hàng dòng mẹ (ADN ty thể)"]
    : ["Xét nghiệm ADN Cha – Con", "Xét nghiệm ADN Mẹ – Con", "Xét nghiệm ADN Họ hàng dòng cha", "Xét nghiệm ADN Họ hàng dòng mẹ (ADN ty thể)"]
  ...
  const handleSubjectChange = (index: number, field: string, value: any) => {
    setTestSubjects(prev => prev.map((subject, i) => 
      i === index ? { ...subject, [field]: value } : subject
    ))
  }

  const addTestSubject = () => {
    setTestSubjects(prev => [...prev, {
      fullName: '',
      birthYear: '',
      gender: '',
      relationship: '',
  ...
  const handleFileUpload = (field: string, file: File | null, subjectIndex?: number) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const requiredFields = ['appointmentDate', 'appointmentTime', 'selectedService', 'fullName', 'birthDate', 'gender', 'idNumber', 'phoneNumber']
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleFileUpload('signature', e.target.files?.[0] || null, index)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                        />
                      </div>
                    </div>
              ...
              <div className="border-t pt-6">
                ...
                <ol className="list-decimal list-inside space-y-2 text-sm text-gray-700 mb-4">
                  ...
                  <li>Những trường hợp sinh đôi, người ghép tủy, nhận mẫu, nếu không khai báo trung thực sẽ bị phạt gấp 2 lần lệ phí đã nộp.</li>
                  <li>
                    Tôi đã đọc và chấp nhận các{' '}
                    <Link to="/terms" className="text-[var(--primary)] hover:underline">
                      điều khoản của trung tâm GenUnity
                    </Link>
                    {' '}và tôi đồng ý để Viện thực hiện các phân tích với các mẫu trên. Nếu vi phạm, tôi xin chịu hoàn toàn trách nhiệm trước pháp luật.
                  </li>
                </ol>
